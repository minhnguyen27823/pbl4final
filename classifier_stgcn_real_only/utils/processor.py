...
def plot_confusion_matrix(confusion_matrix, title='CM', fontsize=20):
    # Sử dụng giao diện của seaborn
    sns.set_theme()

    rcParams['text.usetex'] = False  # Tắt LaTeX nếu chưa cài đặt
    ...
    color_names = ['goldenrod', 'bisque', 'paleturquoise', 'limegreen']
    colors = [mcolors.to_rgba(mcolors.CSS4_COLORS[name], 1.0) for name in color_names]

    # Tạo văn bản hiển thị trong bảng
    cell_text = [[f"{int(x)}" for x in row] for row in confusion_matrix]
    ...
    fig.savefig(f"{save_path}/{title}.png", bbox_inches='tight')
...
class Processor(object):
    ...
    def load_best_model(self):
        ...
        filename = "D:/PBL4/STEP-master/STEP-master/classifier_stgcn_real_only/model_classifier_stgcn/features/epoch395_acc81.25_model.pth.tar"
        print(filename)
